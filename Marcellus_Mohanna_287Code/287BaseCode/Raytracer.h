...
struct RayTracer {
	...
	void raytraceSceneAA(FrameBuffer &frameBuffer, int depth,
						...
						int N) const;
	void raytraceSceneAntiAliasing(FrameBuffer &frameBuffer, int depth, 
						const IScene &theScene) const;
protected:
	color traceIndividualRay(const Ray &ray, const IScene &theScene, int recursionLevel) const;
	color colorCalculations(const HitRecord hit, const Ray &ray, const IScene &theScene, int recursionLevel) const;
};
