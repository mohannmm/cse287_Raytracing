...
color PositionalLight::illuminate(const Material &material, 
	...
	glm::vec3 v = glm::normalize(interceptWorldCoords - eyeWorldCoord);
	...
	if (result == white) {
		color X = totalColor(material, lightColorComponents, v, normal, this->lightPosition, interceptWorldCoords, this->attenuationIsTurnedOn, this->attenuationParams);
