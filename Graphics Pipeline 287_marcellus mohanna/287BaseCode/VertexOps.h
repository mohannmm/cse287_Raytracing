...
public:
	...
	static void VertexOps::render(FrameBuffer &frameBuffer, const std::vector<VertexData> verts,
								const std::vector<LightSourcePtr> &lights,
								const glm::mat4 &TM);
	static void setViewport(float left, float right, float bottom, float top);
	static void setViewport(const BoundingBox &vp);
protected:
	static BoundingBox viewport;			//!< the currently active viewport
