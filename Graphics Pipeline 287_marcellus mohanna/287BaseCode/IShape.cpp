...
ICylinderYClosed::ICylinderYClosed(const glm::vec3 &pos, float rad, float len)
	: ICylinderY(pos, rad, len),
	top (pos + glm::vec3(0, len / 2.0f, 0), glm::vec3(0,1,0), rad),
	btm (pos - glm::vec3(0, len / 2.0f, 0), glm::vec3(0,-1,0), rad) {
	std::cout << "pos: " << pos << std::endl;
	std::cout << "length: " << len << std::endl;
	std::cout << "rad: " << rad << std::endl;
	std::cout << "top pos: " << top.center << std::endl;
	std::cout << "btm pos: " << btm.center << std::endl;
}
...
void ICylinderYClosed::findClosestIntersection(const Ray &ray, HitRecord &hit) const {
	const glm::vec3 &rayOrigin = ray.origin;
	const glm::vec3 &rayDirection = ray.direction;
	HitRecord hits[2], topDiskHit, btmDiskHit;

	int numHits = ICylinder::findIntersections(ray, hits);

		btm.findClosestIntersection(ray, topDiskHit);
		top.findClosestIntersection(ray, btmDiskHit);

	for (int i = 0; i < numHits; i++) {
		if (hits[i].interceptPoint.y < center.y + length / 2 &&
			hits[i].interceptPoint.y > center.y - length / 2) {
			
			float topD = glm::distance(topDiskHit.interceptPoint, rayOrigin);
			float btmD = glm::distance(topDiskHit.interceptPoint, rayOrigin);
			float cylD = glm::distance(hits[i].interceptPoint, rayOrigin);

			if (topDiskHit.t < FLT_MAX && topD < btmD && topD < cylD) { //top disk closer
				hit = topDiskHit;
			}
			else if (btmDiskHit.t < FLT_MAX && btmD < topD && btmD < cylD) { // btm disk closer
				hit = btmDiskHit;
			}
			else { //side of cylinder closer
				hit = hits[i];
			}
			return;
		}
...
void ICylinderYClosed::getTexCoords(const glm::vec3 &pt, float &u, float &v) const {
