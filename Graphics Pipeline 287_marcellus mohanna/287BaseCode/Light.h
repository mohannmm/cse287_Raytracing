...
struct LightSource {
	...
	virtual color illuminate(const HitRecord &hit, const glm::vec3 &viewingDir,
								const Frame &eyeFrame, bool inShadow) const = 0;
	virtual color illuminate(const Material &material,
							const glm::vec3 &normal,
							const glm::vec3 &interceptWorldCoords,
							const glm::vec3 &eyeWorldCoord,
							const glm::mat4 &viewingMatrix) const = 0;
	};
...
struct AmbientLight : public LightSource {
	color ambientColor;
	AmbientLight(const color &lightColor = white) : LightSource(), ambientColor(lightColor) {
	}
	virtual color illuminate(const HitRecord &hit, const glm::vec3 &viewingDir,
