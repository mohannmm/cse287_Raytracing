...
void FragmentOps::processFragment(FrameBuffer &frameBuffer, const glm::vec3 &eyePositionInWorldCoords,
	...
	bool passDepthTest = Z < frameBuffer.getDepth(X,Y);
	if (passDepthTest) {
		color C = lights[0]->illuminate(fragment.material, fragment.worldNormal,
			fragment.worldPosition,eyePositionInWorldCoords, viewingMatrix);
