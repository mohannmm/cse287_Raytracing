...
BoundingBox VertexOps::viewport(0, WINDOW_WIDTH - 1, 0, WINDOW_HEIGHT - 1);
...
void VertexOps::applyLighting(const std::vector<LightSourcePtr> &lights, std::vector<VertexData> &worldCoords) {
	glm::vec3 eyePositionInWorldCoords = glm::inverse(VertexOps::viewingTransformation)[3].xyz;
	...
	for (unsigned int i = 0; i < worldCoords.size(); i++) {
		...
		for (unsigned int j = 0; j < lights.size(); j++) {
			totalLight += lights[j]->illuminate(vert.material, vert.normal, vert.worldPosition,
				eyePositionInWorldCoords, viewingTransformation);
		}
		worldCoords[i].material = totalLight;
		worldCoords[i].material.alpha = alpha;
	}
}
...
void VertexOps::processTriangleVertices(FrameBuffer &frameBuffer, const glm::vec3 &eyePos,
	...
	// ZZZ Do this after removing backfacing triangles
	//applyLighting(lights, worldCoords);	// Perform lighting calculations in world coordinates
	...
	for (VertexData &vd : windowCoords) {
		vd.position.x = glm::clamp(vd.position.x, viewport.lx, viewport.rx);
		vd.position.y = glm::clamp(vd.position.y, viewport.ly, viewport.ry);
	}
...
void VertexOps::setViewport(float left, float right, float bottom, float top) {
	setViewport(BoundingBox(left, right, bottom, top));
}
...
void VertexOps::setViewport(const BoundingBox &vp) {
	viewport = vp;
...
void VertexOps::setViewportTransformation() {
	VertexOps::viewportTransformation = T(VertexOps::viewport.lx, VertexOps::viewport.ly, 0.0f) *
		S(VertexOps::viewport.width() / VertexOps::ndc.width(),
			VertexOps::viewport.height() / VertexOps::ndc.height(), 1.0f) *
